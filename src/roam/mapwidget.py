...
from roam.maptools import MoveTool, InfoTool, EditTool, PointTool, TouchMapTool, AnnotationTool
...
class MapWidget(Ui_CanvasWidget, QMainWindow):
    def __init__(self, parent=None):
        ...
        self.editgroup.addAction(self.actionAnnotation)
        ...
        self.actionAnnotation.triggered.connect(self.emit_show_annot)
        self.actionLayerList.triggered.connect(self.showLayerList)
        ...
        RoamEvents.closeAnno.connect(self.closeActAnnotation)
        RoamEvents.setAnnoDisabled.connect(self.setActAnnoDisabled)
        RoamEvents.setAnnoEnabled.connect(self.setActAnnoEnabled)
        RoamEvents.closeLayerList.connect(self.closeLayerList)
    ...
    def closeActAnnotation(self):
        self.actionAnnotation.setChecked(False)
        self.actionInfo.setChecked(True)

    def setActAnnoDisabled(self):
        self.actionAnnotation.setEnabled(False)

    def setActAnnoEnabled(self):
        self.actionAnnotation.setEnabled(True)
    ...
    def connectButtons(self):
        ...
        self.annotationTool = AnnotationTool(self.canvas)
        ...
        connectAction(self.actionAnnotation, self.annotationTool)
    ...
    def emit_show_annot(self):
        RoamEvents.showAnnotationUi.emit()

    def showLayerList(self):
        RoamEvents.showLayerListUi.emit(self.canvaslayers)

    def closeLayerList(self):
        self.actionLayerList.setChecked(False)
